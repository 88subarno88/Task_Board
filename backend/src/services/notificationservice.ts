...
export const notifyStatusChange = async (
  ...
  const usersToNotify = [issue.assigneeId, issue.reporterId].filter(
    (id): id is string => id !== null && id !== changedById
  ); // exclude person who made change
...
export const notifyComment = async (issueId: string, commenterId: string) => {
  ...
  const usersToNotify = [issue.assigneeId, issue.reporterId].filter(
    (id): id is string => id !== null && id !== commenterId
  );
